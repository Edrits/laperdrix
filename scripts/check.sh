...
if [ "$CODE" = "200" ]; then
  ...
  if [ "$N" -ge 12 ]; then
    pass "All 12 photographs are in the bucket"
  else
    fail "Only $N of 12 photographs could be signed"
    note "Check the names in trip-photos: group, lake, abbey, chair, mirror, heads,"
    note "hens, table, window, garden, house, pool (.jpg)"
  fi
else
  fail "Photographs are not available ($CODE)"
  note "Upload the twelve files from photos/ into the trip-photos bucket."
fi
